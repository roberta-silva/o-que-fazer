...
export default function Home() {
  ...
  async function buscar() {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : 'Erro ao buscar. Tente novamente.',
      );
    } finally {
      setLoading(false);
    }
    ...
    <main className={styles.main}>
      <div className={styles.hero}>
        ...
        <p className={styles.desc}>
          Informe uma cidade e uma data — a IA sugere atividades personalizadas
          para você.
        </p>
      </div>
        ...
        <div className={styles.results}>
          {loading && (
            <div className={styles.loadingState}>
              ...
              <p>
                Buscando atividades em <strong>{location}</strong>...
              </p>
            </div>
          )}
          ...
          {!loading && atividades.length > 0 && (
            <>
              <p className={styles.resultsHeader}>
                {atividades.length} sugestões para <strong>{location}</strong> —{' '}
                {dateLabel}
              </p>
              <div className={styles.cards}>
                {atividades.map((a, i) => (
                  ...
                  >
                    <span
                      className={`${styles.tag} ${getCategoriaClass(a.categoria)}`}
                    >
                      {a.categoria}
                    </span>
                    ...
                    {a.lugar && (
                      <a
                        href={
                          ...
                          encodeURIComponent(a.lugar + ' ' + location)
                        }
                        target="_blank"
      ...
      <footer className={styles.footer}>Feito com Next.js + Claude API</footer>
    </main>
  );
}
